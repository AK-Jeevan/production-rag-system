...
class TestQueryRoute:
    ...
    async def test_query_success(self, mock_rag_result):
        with patch("app.api.routes.rag_service.query", return_value=mock_rag_result):
    ...
    async def test_query_service_error_returns_500(self):
        with patch("app.api.routes.rag_service.query", side_effect=Exception("LLM failed")):
            async with AsyncClient(
                transport=ASGITransport(app=app),
                base_url="http://test"
            ) as client:
    ...
    async def test_query_value_error_returns_422(self):
        with patch("app.api.routes.rag_service.query", side_effect=ValueError("Empty question")):
            async with AsyncClient(
                transport=ASGITransport(app=app),
                base_url="http://test"
            ) as client:
